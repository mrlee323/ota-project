...
|---|---|:--:|:--:|---|
| ~~Cerebras~~ | ~~1M 토큰/일~~ | **카드 필요** | — | **제외** — 추론 호출이 402. 무료 티어 활성화에 카드 등록을 요구한다 (2026-09-05 실측) |
| **Google AI Studio** | 넉넉함 (분당 한도 있음) | 불필요 | ✅ | OpenAI 호환 엔드포인트 제공. 이 레포가 이미 Gemini 를 쓴다 |
...
| 용도 | 공급자 | 이유 |
|---|---|---|
| **기본** | **Google AI Studio (`gemini-2.5-flash`)** | 카드 없이 즉시 발급 · OpenAI 호환 · 이미지 배관과 같은 계정 |
...
> **처음 추천은 Cerebras 였고 틀렸다.** 조사 시점의 «무료 1M/일» 정보와 달리
> 실제 계정에서는 모든 모델이 402 를 돌려줬고, 활성화에 카드 등록을 요구했다.
> `baseURL` 을 환경변수로 뺀 설계 덕에 교체는 환경변수 세 줄이었다 —
> **공급자를 코드에 박지 않은 것이 여기서 값을 했다.**

### `max_tokens` 를 반드시 넉넉히 준다

thinking 을 쓰는 모델(gemini-2.5-flash 등)은 여기가 좁으면 JSON 이 중간에 잘린다.
그러면 파싱 실패로 «모델이 스키마를 못 지켰다» 처럼 보여 **통과율이 왜곡된다.**
실제로 `max_tokens: 200` 에서는 `"Here is the JSON requested:"` 만 돌아왔다. 4000 으로 둔다.
...
`intent` **없이** 페이지 문맥만으로 생성했다.

| 넣은 문맥 | 이미지에 나온 것 |
|---|---|
| 「가을 오사카 특가」 | 단풍, 가을 색조 |
| 「도톤보리 인근 숙소」 | **운하와 도심 건물** |
| 배경색 `#E8543F` | 주홍 톤 |
| 슬롯 = 본문 | 배경용 얕은 심도 |

합성된 프롬프트에도 문맥이 그대로 실렸다 —
*"autumn in Osaka … vibrant red and orange autumn leaves … nearby urban canal"*.
제약 4줄(글자·사람·실제 건물·정보 없음)도 지켜졌다.

**담당자가 프롬프트를 한 글자도 쓰지 않았다.** 그게 이 설계의 요점이다.

> **한계** — 대조군(문맥 없음)은 무료 티어 분당 한도(429)로 못 돌렸다.
> 문맥이 프롬프트에 실리는 인과는 확인됐지만, «문맥이 없으면 더 나쁘다» 를
> 나란히 놓고 보여준 것은 아니다.

### 이미지 공급자는 바뀐다 — 환경변수로 뺀다

작업 중에 **기존 이미지 생성이 이미 죽어 있던 것**을 발견했다.
Hugging Face 가 자체 추론(`hf-inference`)을 내리고 외부 공급자 라우팅으로 옮겨,
예전 경로가 전부 **410** 을 돌려준다 (쇼케이스 이미지 생성도 같이 죽어 있었다).

```
router.huggingface.co/hf-inference/models/...      → 410 (전부)
router.huggingface.co/nscale/v1/images/generations → 200 ✅
```

`HF_IMAGE_PROVIDER` · `HF_IMAGE_MODEL` 을 환경변수로 뺐다.
어느 공급자가 살아 있는지는 이걸로 확인한다:
...
| 지표 | 목표 | **실측 (2026-09-05 · 20건)** |
|---|---|---|
| 1차 스키마 통과율 | ≥ 70% | **85% (17/20)** ✅ |
| 재요청 포함 최종 | ≥ 90% | **95% (19/20)** ✅ |
| 실패 사유 분포 | 어디를 고칠지 알려주는 유일한 근거 | **전부 429(무료 티어 분당 한도)** |

**실패 3건은 스키마 문제가 아니다.** 전부 rate limit 이라 요청을 못 보낸 것이다.
템플릿별로 보면 앞 16건(t1·t2·t3)은 **전부 1차 통과**했고 마지막 t4 구간에서만 한도에 걸렸다 —
스키마 준수율 자체는 사실상 100% 다.
...
> **배치 생성에는 딜레이가 필요하다.** 무료 티어 분당 한도가 실측을 방해했다.
