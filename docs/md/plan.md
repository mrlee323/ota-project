...
**고쳤다 (§12).** 원인은 템플릿이 아니라 모듈 샘플이었다 — 템플릿은 값을 안 들고 있고,
`blockFromDef` 가 `def.sample` 을 통째로 복사한다.
...
## 12. 샘플 문구가 실물이 되던 것 (고침)

§10 의 2번. 증상은 「제목이 안 맞는다」였지만 원인은 더 넓었다.

### 원인

템플릿(`t3-hub`)은 `{ moduleType: "hero" }` 만 들고 있고 값이 없다.
값을 채운 건 `blockFromDef` 가 복사하는 **모듈 샘플**이다:

```
hero.sample.title    = "가을 오사카 특가"
hero.sample.subtitle = "지금 예약하면 최대 30% 할인"
```

제목이 어긋난 건 증상 하나일 뿐이고, **근거 없는 할인율이 실제 기획전 문구로 발행될 수 있었다.**
샘플은 캔버스에서 「이런 모양입니다」를 보여주려고 있는 것이고, 거기선 사람이 보고 지운다.
MCP 초안에는 지울 사람이 없다.

### 고친 것

1. **`materialize` 가 «내용» 칸의 샘플을 지운다.** 넘어오지 않은 칸만이고,
   모양(preset·fixed) 기본값은 남는다. 내용/모양 판별은 이미 있던 `isContentField` 를 그대로 쓴다
   (묶음 복제가 쓰던 것과 같은 정의다 — 판별 기준이 두 벌이 되면 안 된다)
2. **받은 `title` 을 첫 블록 제목 칸에 넣는다.** 왕복 한 번이 줄어든다 (Q-M6 의 6번 중 1번)
3. **초안에 발행 기준을 들이대지 않는다.** 덜 채웠다고 `create_md_draft` 를 거절하면
   AI 는 칸을 메우려고 문구를 지어낸다 — 사고가 더 커진다. 저장은 통과시키고
   `needsInput` 으로 돌려준 뒤, 막는 건 `publishBlockers` 가 사람 앞에서 한다 (FR-2.5 와 같은 기준)

### 곁가지

`materialize` 는 모듈 정의로 블록을 다시 세우는 순수 함수인데 `server-only` 파일에 있어
테스트가 열 수 없었다. `domain/md/materialize.ts` 로 옮겼다.

### 고정

`materialize.test.ts` — **모듈 전체를 돌면서** 값 없이 세운 블록에 내용 칸이 비어 있는지 본다.
모듈을 새로 추가해도 자동으로 검사 대상이 된다.

### 마지막 관문도 막았다 — 새는 곳이 셋이었다

MCP 만 고치면 원인의 한 갈래만 막힌다. 서브에이전트로 `blockFromDef`·`def.sample` 의
모든 경로를 훑어 보니 샘플이 실물이 될 수 있는 길이 셋이었다:

| 경로 | 성격 | 처리 |
|---|---|---|
| MCP `create_md_draft` · `update_md_draft` | 사람이 없다 | `materialize` 에서 지운다 |
| 캔버스 모듈 추가 · 템플릿 얹기 | 사람이 화면에서 본다 (FR-9.6) | 그대로 — 샘플이 목적이다 |
| L1 요청서 변환 (`mdAiService.assemble`) | 저장하지 않고 캔버스로 돌려주고, UI 가 «샘플로 채웠습니다» 를 알린다 | 그대로 |

그런데 **셋 다 `publishBlockers` 하나를 지난다.** 그리고 그게 «빈 칸» 만 보고 있었다 —
샘플을 고치지 않으면 통과한다. 그래서 첫 사고가 프로덕션까지 갔다.

`sampleAsIs()` 를 넣었다. 모듈 샘플과 **글자 그대로 같은 내용 칸**이 있으면 발행을 막고
어느 칸인지 알려준다. 모양(preset·fixed) 칸은 보지 않는다 — 정해진 값 중 고르는 것이라
샘플과 같은 게 정상이다.

곁가지로 드러난 것: L1 은 이미지 필드를 LLM 에게 묻지 않으므로(`aiSchema.ts` — 지어낸 URL 은
깨진 이미지가 된다) **샘플 `imageUrl` 이 항상 남는다.** 이제 그게 발행을 막는다.
담당자는 P5 의 이미지 생성이나 업로드로 바꿔야 한다 — placehold.co 자리표시자가
기획전으로 공개되던 길이 닫혔다.
