...
그 문맥을 프롬프트에 넣으면 어울리는 이미지가 나올 여지가 있다. (§5)
...
| `generate_module_image` | 문맥을 넣은 이미지 생성 → 저장 → URL 반환 (§5) |

### 절대 만들지 않는 도구

`publish` · `delete` · `archive` · 노출 기간 변경 · 외부 전송
...
> 요구사항 §2 는 「이미지 생성 AI」를 비목표로 뒀었다. **조건부로 범위에 넣는다** (§8).
> 조건은 아래 세 가지이고, 하나라도 빠지면 «자유롭게 나오는 이미지» 로 되돌아간다.
...
| 제약의 출처 | 프롬프트에 넣을 것 |
|---|---|
| **디자인시스템 토큰** | 브랜드 색, 톤, 분위기 |
| **슬롯** | 종횡비·크기, 어디에 놓이는지 (히어로 배경 / 카드 썸네일 / 구간 배경) |
| **인접 모듈의 값** | 기획전 제목, 호텔명, 지역, 테마 문구, 주변 블록의 성격 |
| **페이지 전체** | 이미 쓰인 이미지들의 톤 — 한 페이지 안에서 따로 놀지 않게 |

```
generate_module_image({
  pageId, blockId,          // ← 여기서 슬롯·인접 모듈·기존 톤을 서버가 직접 읽는다
  intent: "가을 오사카 밤거리"
})
```

**핵심은 `intent` 가 아니라 `pageId`/`blockId` 다.**
사용자가 프롬프트를 잘 쓰게 만드는 게 아니라, **서버가 문맥을 붙인다.**
그래야 담당자가 프롬프트 엔지니어링을 몰라도 어울리는 결과가 나온다.

### 못 미룰 제약 — 이미지 안에 글자를 넣지 않는다

**이게 가장 중요하다.** 실사 F1 의 결론이 «텍스트가 이미지 안에 갇혀 있다» 였는데,
AI 이미지 생성이 글자 든 이미지를 만들기 시작하면 **이 프로젝트가 고치려던 문제를 그대로 재생산한다.**

| 규칙 | 이유 |
|---|---|
| 이미지에 텍스트·로고·가격·버튼을 넣지 않는다 | 텍스트는 모듈이 그린다. NFR-3(검색 가능) · FR-6.2 |
| 사람 얼굴을 넣지 않는다 | 초상권 · 품질 편차 |
| 실제 호텔 건물을 «묘사» 하지 않는다 | 실물과 다른 이미지는 허위 표시 |
| 배경·질감·분위기용으로만 쓴다 | 정보는 데이터로, 이미지는 분위기로 |

이 네 줄은 `generate_module_image` 의 시스템 프롬프트에 **고정 문구**로 들어간다.

### 흐름

```
① 서버가 문맥 수집   슬롯 정보 + 토큰 + 인접 모듈 값 + 페이지 톤
② 프롬프트 합성      문맥 + intent + 고정 제약 4줄
③ 생성              기존 infrastructure/imageGeneration 재사용
④ 저장              Supabase storage → URL
⑤ 반환              URL + 편집 URL. draft 에 넣되 자동 발행 안 함
```

**사람 확인이 필수다.** 생성된 이미지는 draft 에만 들어가고,
담당자가 캔버스에서 보고 쓸지 정한다. 여기서만은 AI 를 믿지 않는다.

### 비용

이미지 생성은 **MCP 호출자의 토큰이 아니라 우리 비용**이다 (서버가 생성 API 를 부른다).
→ 토큰당 한도가 필요하다 (§6). 텍스트 조립과 달리 여기는 비용이 우리 쪽에 남는다.
...
| 이미지에 텍스트·얼굴 | 시스템 프롬프트 고정 제약 + 사람 확인 (§5) |
...
| 문서 | 변경 |
|---|---|
| `requirements.md §2 비목표` | 「이미지 생성 AI」 제외 → **조건부 포함**. 조건은 §5 의 문맥 3종 + 제약 4줄 |
| `requirements.md FR-1.5` | 필드 `description` 외에 모듈 단위 **`whenToUse`** 추가 (§4) |
| `requirements.md §9` | **Q6** — 운영 경로를 MCP 로 옮기고 P4 를 측정용 최소로 축소할 것인가 |
| `design.md` | `moduleDefSchema` 에 `whenToUse` 필드 |
| `llm.md §8` | 배치 생성 금지 제약은 MCP 경로에 해당 없음 |
...
|---|---|
| Bearer 가능 | 그대로 간다 |
| OAuth 만 됨 | `protectedResourceHandler` 로 OAuth 디스커버리 추가 (§6). 막히는 게 아니라 한 단계 늘어남 |
...
- `create/update` 는 `validateBlock` 을 반드시 통과 (§7)
...
`generate_module_image({ pageId, blockId, intent })` — §5 그대로.

- 문맥 수집: 슬롯(종횡비·위치) + 토큰 + 인접 모듈 값 + 페이지 톤
- 고정 제약 4줄(텍스트·얼굴·실제 건물·정보 금지)을 시스템 프롬프트에
- 저장: Supabase storage → URL 반환. **draft 에만** 들어간다
- 토큰당 일일 상한

**검증 — 같은 `intent` 로 문맥 있음/없음을 나란히 놓는다.** §1 P3 가설의 유일한 검증법. → Q-M4

### M4 · 인증·감사 정식화

S0 의 단일 토큰을 걷어낸다.

- [ ] `md_mcp_tokens` (§6) + 어드민 발급·폐기 화면
...
| # | 질문 | 왜 중요한가 | 어디서 |
|---|---|---|---|
| **Q-M1** | ChatGPT 커넥터에 **Bearer 토큰**을 넣을 수 있나, OAuth 만 되나? | 안 되면 `protectedResourceHandler` 로 OAuth 경로 (§6) | M0 |
...
**권고** — 운영 경로는 MCP, P4 는 **AC-4 측정용 최소 구현**으로 축소.
P4 를 통째로 버리면 AC-4(스키마 1차 통과율 ≥70%)를 잴 수단이 사라진다.
그 숫자는 스키마 설계가 좋았는지 보여주는 유일한 근거라 포기하지 않는다. → Q6

---

## 12. 안 하는 것

- **MCP 서버 안에서 텍스트 LLM 을 부르는 것** — 호출자가 이미 LLM 이다. 비용 이전의 의미가 사라진다
  (이미지 생성은 예외 — 호출자가 못 하는 일이다)
- **발행·삭제 도구** (§4)
- **이미지 업로드 도구** — 파일 전송은 대화보다 UI 가 맞다
- **stdio 전송** — 배포된 서버에 붙을 수 없다. 원격 HTTP 만
- **조직 권한 체계 흉내** — 이 프로젝트가 검증하려는 대상이 아니다 (§6)
