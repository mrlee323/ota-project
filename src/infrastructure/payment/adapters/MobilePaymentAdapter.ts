...
/**
 * Mobile 환경 결제 어댑터
 * 모바일 브라우저 환경에서의 결제 처리를 담당한다.
 * 내부적으로 IPGClient를 주입받아 실제 PG 통신을 위임한다.
 */
export class MobilePaymentAdapter implements IPaymentAdapter {
  readonly environment: PaymentEnvironment = "MOBILE";
  ...
  async requestPayment(context: PaymentContext): Promise<PGResponse> {
    try {
      ...
      return {
        success: false,
        errorCode: "MOBILE_ADAPTER_ERROR",
        errorMessage:
          error instanceof Error
            ? error.message
            : "Mobile 환경 결제 처리 중 알 수 없는 오류가 발생했습니다.",
      };
    }
  }
