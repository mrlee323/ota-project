...
/**
 * PC 환경 결제 어댑터
 * 데스크톱 브라우저 환경에서의 결제 처리를 담당한다.
 * 내부적으로 IPGClient를 주입받아 실제 PG 통신을 위임한다.
 */
export class PCPaymentAdapter implements IPaymentAdapter {
  readonly environment: PaymentEnvironment = "PC";
  ...
  async requestPayment(context: PaymentContext): Promise<PGResponse> {
    try {
      ...
      return {
        success: false,
        errorCode: "PC_ADAPTER_ERROR",
        errorMessage:
          error instanceof Error
            ? error.message
            : "PC 환경 결제 처리 중 알 수 없는 오류가 발생했습니다.",
      };
    }
  }
